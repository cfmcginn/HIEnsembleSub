...
int createEnsembleMapHistID(const std::string inFileName)
{
  ...
  entryTree1_p->Branch("genHIBin", &genHIBin, "genHIBin/i");
  ...
  std::map<UInt_t, std::map<UInt_t, std::vector<Float_t> > > fullTowerPosToSumVect;

  for(UInt_t i = 0; i < nTowers; ++i){
    for(auto const & val : mapEventIDToCounts){
      (fullTowerPosToSumVect[i])[val.first] = {};
    }
  }
  ...
  const ULong64_t nDiv1 = TMath::Max((ULong64_t)1, nEntries1/20);
  std::cout << "Processing tree 1..." << std::endl;
  for(ULong64_t entry = 0; entry < nEntries1; ++entry){
    if(entry%nDiv1 == 0) std::cout << " Entry " << entry << "/" << nEntries1 << std::endl;
    hydTree1_p->GetEntry(entry);

    for(UInt_t i = 0; i < nTowers; ++i){
      (fullTowerPosToSumVect[i])[eventID].push_back(etaPhiSum[i]);
    }
  }

  for(auto const & val : mapEventIDToCounts){
    for(UInt_t i = 0; i < nTowers; ++i){
      std::vector<Float_t> temp = (fullTowerPosToSumVect[i])[val.first];
  ...
  for(UInt_t i = 0; i < nTowers; ++i){
    for(auto const & val : mapEventIDToCounts){
      (fullTowerPosToSumVect[i])[val.first].clear();
    }
  }


  const ULong64_t nDiv2 = TMath::Max((ULong64_t)1, nEntries2/20);
  std::cout << "Processing tree 2..." << std::endl;
  for(ULong64_t entry = 0; entry < nEntries2; ++entry){
