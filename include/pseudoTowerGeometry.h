...
class pseudoTowGeo{
 public:
  ...
  const static int nEtaTowHalf = 41;
  const double etaTowBoundsHalf[nEtaTowHalf+1] = {-5.191, -4.716, -4.363, -4.013, -3.664, -3.314, -3, -2.65, -2.322, -2.043, -1.83, -1.653, -1.479, -1.305, -1.131, -0.957, -0.783, -0.609, -0.435, -0.261, -0.087, 0.087, 0.261, 0.435, 0.609, 0.783, 0.957, 1.131, 1.305, 1.479, 1.653, 1.830, 2.043, 2.322, 2.650, 3.000, 3.314, 3.664, 4.013, 4.363, 4.716, 5.191};
  ...
  const int nTowInPhiHalf[nEtaTowHalf] = {9, 18, 18, 18, 18, 18, 18, 18, 18, 18, 18, 36, 36, 36, 36, 36, 36, 36, 36, 36, 36, 36, 36, 36, 36, 36, 36, 36, 36, 36, 18, 18, 18, 18, 18, 18, 18, 18, 18, 18, 9};
  ...
  std::vector<double> etaTowBoundsHalfVect;
  std::vector<int> nTowInPhiVect;
  std::vector<int> nTowInPhiHalfVect;
  ...
  std::vector<double> getEtaTowBoundsHalf();
  std::vector<int> getNTowInPhiHalf();
...
pseudoTowGeo::pseudoTowGeo()
{
  ...
  etaTowBoundsHalfVect.reserve(nEtaTowHalf+1);
  for(unsigned int iter = 0; iter < nEtaTowHalf+1; ++iter){
    etaTowBoundsHalfVect.push_back(etaTowBoundsHalf[iter]);
  }
  ...
  for(int iter = 0; iter < nEtaTow; ++iter){
    nTowInPhiVect.push_back(nTowInPhi[iter]);
  }

  nTowInPhiHalfVect.reserve(nEtaTowHalf);
  for(int iter = 0; iter < nEtaTowHalf; ++iter){
    nTowInPhiHalfVect.push_back(nTowInPhiHalf[iter]);
  }

  nTowInPhiVect.reserve(nEtaTow/2);
  for(int iter = 0; iter < nEtaTow/2; ++iter){
...
std::vector<double> pseudoTowGeo::getEtaTowBoundsHalf(){return etaTowBoundsHalfVect;}
std::vector<int> pseudoTowGeo::getNTowInPhi(){return nTowInPhiVect;}
std::vector<int> pseudoTowGeo::getNTowInPhiHalf(){return nTowInPhiHalfVect;}
